...
// NEW DATA ARRAYS FOR STRETCH 2 ⬇️
 ...
 function getRandomFlavors(arr1, arr2, arr3, arr4) {
  let allFlavors = arr1.concat(arr2, arr3, arr4);
  let randomFlavors = [];

  for (let i = 0; i < allFlavors.length; i++) {
    let randomNum = Math.floor(Math.random() * allFlavors.length);
    if (randomFlavors.length < 31)
      randomFlavors.push(allFlavors[randomNum]);
        allFlavors.splice(randomNum, 1)
  }
